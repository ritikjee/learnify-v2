...
const SideBar = ({ groupid, userid, mobile }: Props) => {
  ...
  return (
    ...
    >
      {groups.data?.groups && groups.data?.groups?.length > 0 && (
        <DropDown
          title="Groups"
          trigger={
              ...
              <div className="flex gap-x-3 items-center">
                ...
                <img
                  src={`https://ucarecdn.com/${
                    groupInfo?.data?.group?.icon as string
                  }/`}
                  alt="icon"
                  className="w-10 rounded-lg"
                />
                <p className="text-sm">{groupInfo?.data?.group?.name}</p>
              </div>
              <span className="">
                <CarotSort />
              </span>
        ...
        >
          {groups &&
            ...
            groups?.data?.groups.map((item) => (
              <Link
                key={item.id}
                href={`/group/${item.id}/channel/${channels?.data?.channels?.[0].id}`}
              >
                <Button
                  variant="ghost"
      ...
      <div className="flex flex-col gap-y-5">
        <div className="flex justify-between items-center">
          ...
          {userid === groupInfo.data.group?.userId && (
            <Plus
              size={16}
              className={cn(
        ...
        <SideBarMenu
          ...
          loading={isPending}
          groupid={groupid}
          groupUserId={groupInfo?.data?.group?.userId as string}
          userId={userid}
        />
      </div>
    </div>
