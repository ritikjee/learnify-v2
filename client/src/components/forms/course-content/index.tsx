...
export const CourseContentForm = ({
  ...
  const {
    ...
    sectionid,
    data?.data?.section?.content || null,
    data?.data?.section?.JsonContent || null,
    data?.data?.section?.htmlContent || null
  );
  ...
  return groupid === userid ? (
    <form onSubmit={onUpdateContent} className="p-5 flex flex-col" ref={editor}>
      <BlockTextEditor
        ...
        content={JSON.parse(data?.data?.section?.JsonContent)}
        htmlContent={data?.data?.section?.htmlContent || undefined}
        setHtmlContent={setOnHtmlDescription}
        textContent={data?.data?.section?.content || undefined}
    ...
    <HtmlParser html={data?.data?.section?.htmlContent} />
  );
};
