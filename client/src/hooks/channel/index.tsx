...
export const useChannelInfo = () => {
  ...
  const { isPending, mutate, variables } = useMutation({
    ...
    onSuccess: ({ data, error }) => {
      // WIP: fix this
      return toast(data ? "Success" : "Error", {
        description: data ? "Successfully updated channel" : error.message,
      });
    },
    onSettled: async () => {
  ...
  const { mutate, variables, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast(data.data ? "Success" : "Error", {
        description: data.data ? "Post created" : "Error creating post",
      });
    },
    onSettled: async () => {
      return await client.invalidateQueries({
...
export const useLikeChannelPost = (postid: string) => {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      toast(data.data ? "Success" : "Error", {
        description: data.data ? "Post liked" : "Error liking post",
      });
    },
    onSettled: async () => {
...
export const usePostComment = (postid: string) => {
  ...
  const { mutate, variables, isPending } = useMutation({
    ...
    onSuccess: (data) =>
      toast(data?.data ? "Success" : "Error", {
        description: data?.data ? "Comment created" : "Error creating comment",
      }),
    onSettled: async () => {
      return await client.invalidateQueries({
...
export const usePostReply = (commentid: string, postid: string) => {
  ...
  const { mutate, variables, isPending } = useMutation({
    ...
    onMutate: () => reset(),
    onSuccess: (data) => {
      return toast(data?.data ? "Success" : "Error", {
        description: data?.data ? "Reply created" : "Error creating reply",
      });
    },
  });
